...
**Electron 44 升级（分支 `chore/electron-upgrade`，2026-09-11，未合并 main）**：37→44.3.0 跨 7 个大版本。破坏点仅 1 个：`clipboard.writeImage` 移除 → `clipboard.write([new ClipboardItem({ 'image/png': new Blob([bytes]) })])`（多格式剪贴板 API；Buffer 与 BlobPart 泛型不兼容需拷贝进全新 Uint8Array）。验证：typecheck + 89 单测 + dev 真机（9s 就绪）+ CDP 双冒烟（ipc-smoke 10 域 + chat-ui-smoke 5 项）全过。**坑两条**：①pnpm 升级后 Electron 二进制 postinstall 下载失败无提示（dev 报 `Error: Electron uninstall`，node_modules/electron 缺 dist/ 与 path.txt）→ `$env:ELECTRON_MIRROR='https://npmmirror.com/mirrors/electron/'; node node_modules\electron\install.js` 补救（直连 GitHub binary CDN 常年不稳，镜像一次过）；②**better-sqlite3 v13 是跨 ABI 构建**——node 24（单测）与 Electron 44（dev）同时加载同一 binary 均成功，`rebuild:native`（electron-rebuild/node-gyp）本机失败但**无需成功**，脚本已过时可在后续清理。合并此分支时注意：CI（ubuntu）与发版 workflow 会拉新 electron，若 CI 二进制下载失败给 job 配 ELECTRON_MIRROR。
