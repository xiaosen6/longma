...
- **0.2.3 已发版**（2026-08-28）：用量仪表盘完整形态 + 撤思考 chip。EBUSY 预热步不够，release.yml win 打包已改 3 次重试循环（每次清半成品 dist）后全绿。另：本机对 github release 资产 CDN 直连常断，**下载资产走 api.github.com 的 asset 端点**（带 token + Accept: application/octet-stream + curl -C - 续传）可稳定慢速拉通——D 盘大文件下载都用这招。：思考等级 chip、Canvas 开关钉窗口右上、用量历史卡、（0.2.1 的视觉 Cindy 化/侧栏拖拽等也首次进正式版——0.2.1 与 0.2.2 同日连发）。**发版坑 +1：EBUSY**——electron-builder 拷 fresh 解包的 cua-driver 二进制被 Defender 锁定，CI 连挂两次；workflow 已加「下载后 hash 全量预热」步（aec4bba），tag 必须含该 commit（重跑复用旧 tag commit 不带修复）。
