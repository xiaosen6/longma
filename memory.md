...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.2.23 | 09-14 | **用户消息就地编辑重发**（hover 气泡出「编辑」→ 就地 textarea → 发送=删本条及之后全部+重发，editAndResendUserMessage）+ **输入卡对齐 Cindy**（附件回形针→Plus；权限+知识库留底部行；FolderPickerChip 移到输入卡下方一行左侧对齐 Cindy「AI OS」位；粘贴 ≥10 行或 ≥800 字符折叠为「粘贴的文本(N 行)」附件——落盘 pasted-<ts>.txt 唯一名；输入卡圆角 12→16px） |
...
| 0.2.20 | 09-12 | **已发布**（CI 绿/Release published 12 资产/latest.yml 已验/用户机覆盖装冒烟过：0.2.20.0+asar hash+title=LongMa+IPC 10 域 PASS）。**知识库引用溯源角标**（@点名注入时 kbRefs 随消息落库零迁移，正文 [n] 渲染上标角标，点击展开原文块卡片，历史会话可溯源）+ **检索长句召回修复**（老缺陷：整句直接 FTS MATCH=短语匹配，长句/自然问题召回恒 0——@注入对正常提问基本失效；query.ts 拆词+OR+中文长词滑窗 4 字子短语+尾窗，英文原样不滑窗；实测 0→3 条）+ E2E kb-cite-e2e.mjs。坑：杀 dev 必须整树杀（CIM 只杀直接子会漏 electron 孙进程——旧实例占单实例锁+CDP，新实例静默退出，探针连旧代码误判"没生效"） |
| 0.2.19 | 09-11 | **已发布**（CI 绿/Release published 12 资产/latest.yml 已验/用户机覆盖装真机冒烟过：ProductVersion+asar hash+Electron 44 Chromium152+品牌 title=LongMa+IPC 10 域+无崩溃；删 tag 重推后 release 转 draft 已按 0.2.8 规矩 edit --draft=false 转正）。**Electron 37→44.3.0**（唯一破坏点 clipboard.writeImage→ClipboardItem；坑：electron@44 **移除包内 postinstall**——pnpm install 后 dist 不自动下载，dev 报 Error: Electron uninstall、CI 报 electronDist does not exist（EBUSY retry 标签是误标），修复=tools/electron-ensure.cjs 挂 desktop postinstall，SKIP_BINARY_DOWNLOAD=1 跳过保 ci.yml、ELECTRON_MIRROR 透传；better-sqlite3 v13 跨 ABI 无需 rebuild）+ **知识库索引中断恢复** + **IPC 拆分**（register.ts→session-core+9 域 handlers）+ **ChatPage 拆分**（→pages/chat/ 4 hooks+2 组件）+ **测试补强 77→89**（strip-only 不支持参数属性、PS5.1 中文 mojibake 两坑）。**发版闸门新坑**：本地打包必须用根命令 `pnpm dist:win`——`--filter fundet-desktop dist:win` 跳过 build 前置，把 out/ 残留（本例 fundet 品牌 renderer）打进包，装机后 title=Fundet 险些带病发版；产物品牌验证=CDP `document.title==='LongMa'`+asar hash 对比 win-unpacked |
...
**Electron 44 升级（2026-09-11 已合并 main，commit 66632d2）**：37→44.3.0 跨 7 个大版本。破坏点仅 1 个：`clipboard.writeImage` 移除 → `clipboard.write([new ClipboardItem({ 'image/png': new Blob([bytes]) })])`（多格式剪贴板 API；Buffer 与 BlobPart 泛型不兼容需拷贝进全新 Uint8Array）。验证：typecheck + 89 单测 + dev 真机（9s 就绪）+ CDP 双冒烟（ipc-smoke 10 域 + chat-ui-smoke 5 项）全过。

**知识库引用溯源角标 + 检索召回修复（0.2.19 后、未发版）**：
- **溯源角标**：@点名注入时检索结果以 `kbRefs`（itemName/seq/text/baseId/baseName）随消息**落库**（messages.content JSON 加字段，零迁移；气泡仍只显示原文）；assistant 正文渲染前 `lib/kbCitation.ts` 把 `[n]`（n≤引用数）替换成 `[n](#kb-n)`，AssistantMessage 的 a 组件拦截渲染为上标小角标，点击在消息底部展开原文块卡片（来源文件/块号/库），历史会话经 rebuildItems 恢复同样可溯源；kbPrefix 提示词加「用 [1] [2] 编号角标标注」引导。无注入/越界编号原样保留不误伤。
- **检索召回修复（老缺陷）**：原 searchKnowledge 把整句直接给 FTS MATCH=短语匹配，**长句/含标点句召回恒 0**（@点名注入对自然问题基本失效）。`knowledge/query.ts`（纯函数可单测）：标点/空白拆词 → ≥3 字词元进 FTS（引号短语 + OR）→ **≥6 字 CJK 连续段滑窗切 4 字子短语**（步进 2+尾窗，无分词器的中文长句正解；英文词元有天然分隔原样不滑窗）→ <3 字走 LIKE 兜底。实测「推理引擎有什么用？」0 条→3 条。
- E2E：`tools/kb-cite-e2e.mjs`（选库→真实发送→8 角标渲染→弹层展开/收起，KB-CITE-E2E-PASS）。单测 +7（kbCitation 5 + query 6→修正后 6，总 102）。
- **坑**：杀 dev 必须整树杀（CIM 只杀直接子进程会漏 electron 孙进程——旧实例占单实例锁+CDP，新实例静默退出，探针连的一直是旧代码，误判"代码没生效"）。**⚠️ 打包态未冒烟——发版闸门（硬规矩）**：合并后的首次发版（≥v0.2.19）必须先本地 `pnpm dist:win` 打包 → NSIS 覆盖装真机冒烟（验 ProductVersion=44 包、托盘、桌宠透明窗、发消息全链）→ 过了才推 tag；不过则 revert 66632d2 回 37 照常发版。**坑两条**：①pnpm 升级后 Electron 二进制 postinstall 下载失败无提示（dev 报 `Error: Electron uninstall`，node_modules/electron 缺 dist/ 与 path.txt）→ `$env:ELECTRON_MIRROR='https://npmmirror.com/mirrors/electron/'; node node_modules\electron\install.js` 补救（直连 GitHub binary CDN 常年不稳，镜像一次过）；**CI/release.yml 若遇二进制下载失败同样给 job 配 ELECTRON_MIRROR**。②**better-sqlite3 v13 是跨 ABI 构建**——node 24（单测）与 Electron 44（dev）同时加载同一 binary 均成功，`rebuild:native`（electron-rebuild/node-gyp）本机失败但**无需成功**，脚本已过时可清理。
...
- **截图问答（0.2.12 后补，未发版）**：桌宠工具条/右键 → `pet:screenshot-ask` → 主进程 desktopCapturer 截主屏（物理分辨率；先隐藏桌宠防进画面；PNG>8MB 降 1920 宽）→ **主进程直接 `stageBytesIntoWorkDir` 落盘**（目标=最近 updatedAt 会话的 workDir，无会话回落主目录）→ **入待领队列（上限 5）+ `PET_SCREENSHOT` push `{attachment}`**。渲染层 ChatPage：在线收 push + **挂载时 `petTakePendingScreenshots()` 补领**（切设置页等路由时 ChatPage 卸载会丢 push——首次实测「点了没反应」的根因——主进程队列兜底）+ `stageFiles` 把附件归一到当前会话目录（目录内零拷贝引用、目录外拷进 .longma-uploads，修跨会话 workDir 错位）+ focusSignal 聚焦输入框。失败/空源推 `{error}` 文案不静默。focusMainWindow 加 `moveTop()`（Windows 前台锁会拒后台进程 focus）。**坑：electron-vite dev 只 HMR 渲染层，主进程/preload 变更后主窗必须 reload 才有新 preload 方法**。
...
- **0.2.12 后补杂修（同批，未发版）**：①register.ts 只 import 了 getPiVersion 却从未注册 handler——**关于页 pi 版本显示自 0.2.12 起就是坏的**（invoke 报 No handler），已补注册；②initUpdater dev 态早退前先注册 UPDATE_STATUS 空 handler（消 dev 启动报错噪音，打包版不受影响）；③托盘菜单 `'打开 ${brand.name}'` 单引号模板串死文本 bug（brand-copy-hardcode-rule 的又一实例）。
...
**Cindy 移植批次三（0.2.20 后、未发版，对照 2026-09-12 增量）**：①**流式 markdown 块级复用**（对齐 ceb279db0/404573454，Cindy 实测 2 万字符帧 p95 284ms→33ms）——`lib/streamingBlocks.ts` 纯函数切块（空行=块边界、围栏内不切、尾块 unstable），AssistantMessage 流式分支按块 memo 渲染（stable 块零重解析，每 tick 只重解析尾块；尾块挂 word fade 账本，fadeState per 消息持有——块毕业即丢，新块词序从零排队观感正常）；**终版（streaming=false）仍走单 ReactMarkdown 全文**保跨块上下文零差异（Setext/引用延续等边缘语法）。components 提为 buildMdComponents 工厂（流式块与终版共用，角标/图片/mermaid 行为零变化）。②Canvas PDF/HTML iframe 加 `loading="lazy"`。③侧栏排序切换（「会话」标签右侧图标钮：最近活跃/创建时间，localStorage `longma.sidebar-sort`，对齐 #4183）。④Provider 详情区双击=打开编辑对话框（对齐 #4313）。验证：streamingBlocks 单测 7 例（总 109 绿）+ CDP cindy-batch-verify（排序存在/切换）+ kb-cite-e2e 回归（**重构后 15 角标全链 PASS**）。

**行内 code 裸文件名破图修复（0.2.21 后、未发版）**：模型列文件名清单时行内 code（如 `` `inference_p2_0.png` ``）被 code 组件升级为图片预览，裸文件名（缺 `kb_images/` 前缀）按 workDir 拼出 404 → LocalImagePreview 协议 URL + data URL 双失败 → **url 非空永不清除导致破图占位卡死**。修复：LocalImagePreview 双失败进 broken 态 → return null + onBroken 回调；AssistantMessage code 分支包模块级 FilePathCodeChip（持 broken 态），broken 时整体退回普通 code——**增强失败静默退化，绝不破图**。验证：typecheck+109 单测绿；真机定位病灶（原消息无图片语法，纯行内 code 被误升级）。**坑：bash 工具命令结束后会回收其 Start-Process 起的 pnpm 子进程树**——dev 跨命令存活不可靠，验证动作要塞进同一条命令（装机版 exe 直接启动无此问题）。
