...
| 版本 | 日期 | 要点 |
| --- | --- | --- |
| 0.2.20 | 09-12 | **知识库引用溯源角标**（@点名注入时 kbRefs 随消息落库零迁移，正文 [n] 渲染上标角标，点击展开原文块卡片，历史会话可溯源）+ **检索长句召回修复**（老缺陷：整句直接 FTS MATCH=短语匹配，长句/自然问题召回恒 0——@注入对正常提问基本失效；query.ts 拆词+OR+中文长词滑窗 4 字子短语+尾窗，英文原样不滑窗；实测 0→3 条）+ E2E kb-cite-e2e.mjs。坑：杀 dev 必须整树杀（CIM 只杀直接子会漏 electron 孙进程——旧实例占单实例锁+CDP，新实例静默退出，探针连旧代码误判"没生效"） |
