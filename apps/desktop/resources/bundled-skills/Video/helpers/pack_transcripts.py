...
hour of takes in a tenth the tokens of raw word-level JSON and gives
...
def render_markdown(entries: list[tuple[str, float, list[dict]]], silence_threshold: float) -> str:
    ...
    for name, duration, phrases in entries:
        ...
        for p in phrases:
            ...
            if spk is not None:
                # IDs may look like "speaker_0" — strip the prefix for readability
                spk_str = str(spk)
