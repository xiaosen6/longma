...
describe('pi translator', () => {
  ...
  it('Fundet 下游修复:message_end 带 stopReason=error + 空 content + errorMessage → 产出 terminal error 事件', () => {
    const ctx = createPiTranslateContext(noopLogger);
    const { queue, events } = makeQueue();

    translatePiEvent(ev({ type: 'agent_start' }), queue, ctx);
    // 与 401 实测事件帧一致:assistant message content 为空数组,
    // errorMessage 挂在 message 顶层(非 auto_retry 路径)。
    const errorMessage =
      '401 {"error":{"type":"authentication_error","message":"The API Key appears to be invalid."},"type":"error"}';
    translatePiEvent(
      ev({
        type: 'message_end',
        message: {
    ...
    expect(errors).toEqual([
      expect.objectContaining({
        type: 'error',
        source: 'pi',
        data: { message: errorMessage, isTerminal: true },
      }),
    ]);
    // 错误被吞的回归 guard:这种消息不得产出任何 text 事件。
    expect(events.filter((event) => event.type === 'text')).toEqual([]);
    // 错误后 agent_settled 照常收口,UI 恢复可输入(isRunning=false)。
    const lastStatus = events.filter((event) => event.type === 'status').at(-1);
    expect((lastStatus!.data as { isRunning?: boolean }).isRunning).toBe(false);
  });

  it('message_end 带 errorMessage 但无 stopReason 时也产出 terminal error(兜底)', () => {
    const ctx = createPiTranslateContext(noopLogger);
    const { queue, events } = makeQueue();
    ...
    translatePiEvent(
      ev({
        type: 'message_end',
        message: { role: 'assistant', content: [], errorMessage: '400 bad request' },
      }),
      queue,
      ctx,
    );
    ...
    expect(errors[0].data).toEqual({ message: '400 bad request', isTerminal: true });
  });

  it('message_end stopReason=error 且无 errorMessage 时产出带 stopReason 的兜底错误文本', () => {
    const ctx = createPiTranslateContext(noopLogger);
    const { queue, events } = makeQueue();
    ...
    const errors = events.filter((event) => event.type === 'error');
    expect(errors).toHaveLength(1);
    const data = errors[0].data as { message: string; isTerminal: boolean };
    expect(data.isTerminal).toBe(true);
    expect(data.message).toContain('stopReason=error');
  });

  it('正常 message_end(stop)不产出 error 事件(回归 guard)', () => {
    const ctx = createPiTranslateContext(noopLogger);
    const { queue, events } = makeQueue();
